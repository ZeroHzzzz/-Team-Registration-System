...
const handleDelete = async (TeamID: string, Email: string, index: number) => {
    const res = await leader.delete({ Email: Email, TeamID: TeamID });
            ...
            <el-table class="team-table" :data="TableData2.slice((currentPage - 1) * pageSize, currentPage * pageSize)"
                ...
                <el-table-column label="操作" width="300" align="center">
                    <template #default="scope">
                        <el-button type="primary" :icon="Edit" size="mini" @click="edit">编辑</el-button>
                        <el-button @click="handleDelete(scope.row.account, scope.row.TeamId, scope.$index)" type="danger"
                            :icon="Delete" size="mini">注销</el-button>
                        <el-button type="info" :icon="Search" size="mini" @click="confirm">详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
